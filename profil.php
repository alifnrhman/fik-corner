...
<main>
  ...
  <?php
   $columns = "users.No_Handphone, users.Profile_Pic";
   $table = "users";
   $join = "";
   $where ="";
   $orderBy ="";
  ...
  ?>
   ...
   <form class="max-w-lg max-md:mx-auto w-full p-6"> <!-- Menambahkan class untuk menyamakan posisi -->
      <div class="mb-3">
         <input type="hidden" name="Profile_Pic" value="<?= $data[0]['Profile_Pic']; ?>">
         <?php
             echo "<td><img src='img/" . $data[0]['Profile_Pic'] . "' style='max-width: 60px; height: auto;'></td>";
         ?>
         <label for="Profile_Pic" class="text-gray-800 text-[15px] mb-2 block form-label">Foto Profil</label>
      ...
      <div class="mt-4">
         ...
         <div class="relative flex items-center">
            <input name="No_Handphone" type="text" value="<?= $data[0]['No_Handphone']; ?>" required
               class="w-full text-sm text-gray-800 bg-gray-100 focus:bg-transparent px-4 py-3.5 rounded-md outline-blue-600"
